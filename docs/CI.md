...
source (.tf / manifests / schema.sql / seq.json / graph.json)
   ...
   → autolayout.mjs → validate.mjs --strict (gate) → drawio -x (render) → artifact
```

Bonus: the pipeline itself can be the diagram — `ciimports.mjs <repo-root>` turns `.github/workflows/*.yml` + `.gitlab-ci.yml` into a jobs/`needs:` DAG (triggers, matrix sizes, reusable-workflow calls) for the same autolayout → validate → render loop.
...
jobs:
  render:
    ...
    steps:
      ...
      - run: git clone --depth 1 https://github.com/localhost-anon/claude-drawio-skill /tmp/claude-drawio-skill
      ...
      - name: Regenerate, lint, render
        run: |
          S=/tmp/claude-drawio-skill/skills/drawio-skill/scripts
          node $S/tfimports.mjs infra -o /tmp/graph.json
          node $S/autolayout.mjs /tmp/graph.json -o diagrams/infra.drawio
          node $S/validate.mjs diagrams/infra.drawio --strict   # gate: fail on any warning
          export HOME=${HOME:-/tmp}
          xvfb-run -a --server-args="-screen 0 1280x1024x24" \
            drawio -x -f png -e -s 2 -o diagrams/infra.drawio.png \
            diagrams/infra.drawio --disable-gpu --no-sandbox
          node $S/repair_png.mjs diagrams/infra.drawio.png       # fix -e IEND truncation
...
- After any `-e` PNG export, run `repair_png.mjs` — the CLI truncates the IEND chunk (issue #8).
- `--page-index <n>` (1-based) exports one page of a multi-page file (e.g. `c4.mjs` output).
        ...
        run: |
          curl -s -X POST http://localhost:5000/convert \
            ...
            -d "$(node -e 'console.log(JSON.stringify({
                  source: require("fs").readFileSync("diagrams/infra.drawio", "utf8"),
                  format: "png", scale: 2}))')" \
            -o diagrams/infra.png
```

Validation (`validate.mjs --strict`) needs only Node.js — it runs the same way in both options.
