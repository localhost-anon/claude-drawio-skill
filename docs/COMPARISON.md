...
| Auto-layout for large graphs | No — hand-places, overlaps | Yes — vendored elkjs placement, ortho routing, nested containers |
...
| AI/LLM brand logos | No — none | Yes — 321 logos (OpenAI/Claude/Gemini/…) via aiicons.mjs |
...
| **Multi-agent** | ❌ Claude Code only | ⚠️ MCP hosts (Claude, Cursor, VS Code) | ✅ Claude / Gemini / Codex | ❌ Claude Code only |
...
4. **Zero-config, zero-dep** — works in Claude Code with just the `skills/drawio-skill/` directory + draw.io desktop. Pure Node.js (no external deps beyond Node itself), no MCP server, no browser
