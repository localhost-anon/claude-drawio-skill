...
| Final `-e` PNG won't open in image viewers / vision APIs | Run `node <this-skill-dir>/scripts/repair_png.mjs <path>`. draw.io CLI emits `-e` PNGs with an 8-byte truncation at IEND. SVG/PDF unaffected. |
...
```bash
URL=$(node <this-skill-dir>/scripts/encode_drawio_url.mjs --edit diagram.drawio)
