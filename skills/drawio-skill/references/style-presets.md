...
> **Existing diagrams:** these rules apply at generation time. To re-theme a `.drawio` that already exists, run `node scripts/restyle.mjs diagram.drawio --preset <name>` — it applies the palette (hue-mapped), font, and extras without touching layout or edge routing.
