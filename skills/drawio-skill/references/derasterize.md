...
`scripts/raster2drawio.mjs` only turns the extracted JSON into XML.
...
3. **Convert it**:
   ```bash
   node scripts/raster2drawio.mjs graph.json -o out.drawio
   ```
   If any node is missing `x`/`y`, don't guess coordinates by hand — leave
   them out and the script auto-places the whole graph via `autolayout.mjs`,
   noting this on stderr.

4. **Continue the standard workflow**: `validate.mjs` for structural issues,
