...
`scripts/tubemap.mjs` restyles a graph as a **London-Underground-style metro map**:
...
```bash
node <this-skill-dir>/scripts/tubemap.mjs metro.json -o metro.drawio
# then the normal workflow: validate.mjs → preview PNG → self-check → export
```
...
strict hierarchy (use `autolayout.mjs` / a diagram-type preset for those).
...
`tubemap.mjs` honours the grid coordinates you give it — it does **not** solve the
