...
3. Repair the IEND chunk: `node <this-skill-dir>/scripts/repair_png.mjs <preset-name>-sample.png` — the `-e` flag truncates the PNG the same way the main workflow's step 7 does, so the sample needs the same fix to be readable.
