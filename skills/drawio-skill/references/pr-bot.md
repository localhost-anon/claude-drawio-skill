...
- `scripts/prdiff.mjs` — the script. `changed_drawios()` finds what changed
  (`git diff --name-status`); for each file it exports base/head PNGs via the
  draw.io CLI and, for modified files, chains `drawiodiff.mjs` ->
  `autolayout.mjs` -> CLI export into a third diff PNG; `render_markdown()`
  turns all of that into one Markdown report.
- `.github/actions/drawio-diff/action.yml` — a composite action that checks
  out full history, installs Node + the draw.io CLI, runs `prdiff.mjs`, uploads
...
1. Copy `.github/actions/drawio-diff/` and `skills/drawio-skill/` (or at
   least `scripts/prdiff.mjs`, `scripts/drawiodiff.mjs`, `scripts/autolayout.mjs`)
   into your repo.
...
setup documented in `docs/CI.md` "Option A" of this repo: a virtual display
(`xvfb-run`). The composite action installs it by default (latest
`drawio-desktop` `.deb` + `apt-get xvfb`); pass `skip-tool-install: true` if
your runner/container already provides `drawio` on PATH.

If the draw.io CLI is missing (or fails), `prdiff.mjs` does **not** hard-fail
...
```
node skills/drawio-skill/scripts/prdiff.mjs --base origin/main --head HEAD \
