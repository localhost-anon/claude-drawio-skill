...
Instead of computing coordinates by hand in the Generate step, describe the graph as JSON and let `scripts/autolayout.mjs` place the nodes and route the edges, then continue the normal workflow (Export draft → Self-check → …) on the produced `.drawio`. Layout is powered by the bundled `elkjs` engine — no external dependency to install.
...
```bash
node <this-skill-dir>/scripts/autolayout.mjs graph.json -o diagram.drawio
```

It prints `wrote diagram.drawio (N nodes, M edges)` to stderr and writes a normal `.drawio` file. From there, continue at the **Export draft** step of the main workflow (preview PNG with `--width 2000`, self-check, review loop, final export with `-e` + `repair_png.mjs`).
...
| `nodes[].width` / `height` | no | `120` / `60` | Pixels; the layout engine lays out at this real size |
...
- Node positions come from `elkjs` (hierarchical layered layout), converted to draw.io pixels and snapped to the grid (multiples of 10).
- Edges are routed orthogonally: the engine's route is replayed as draw.io waypoints, so edges go **around** nodes instead of through them.
...
Give nodes a `group` key and the script wraps each group in a labeled container (a dashed box with the group title at top) and tells the layout engine to keep that group's nodes together as a cluster. Grouped nodes become children of their container (`parent="<container>"`, relative coordinates); ungrouped nodes stay at the top level. This turns a flat hairball into a "boxes of related modules" architecture view.
...
- Each container box is the bounding box of its members and child boxes plus a uniform padding. The layout engine's cluster margin is set to that same padding, so each box equals its cluster box — kept non-overlapping at **any nesting depth**.
...
`scripts/validate.mjs` is a deterministic structural linter — run it on the produced `.drawio` before the (slower, vision-based) self-check:

```bash
node <this-skill-dir>/scripts/validate.mjs diagram.drawio
```
...
| --- | --- | --- | --- |
| Python | `scripts/pyimports.mjs <dir>` | module / package (`ast`) | intra-project `import` / `from` |
| JS / TS | `scripts/jsimports.mjs <dir>` | source file (`.ts/.tsx/.js/.jsx/.mjs/.cjs`) | resolved relative `import`/`export from`/`require()`/`import()` |
| Go | `scripts/goimports.mjs <dir>` | package (directory, via `go.mod`) | intra-module package import |
| Rust | `scripts/rustimports.mjs <dir>` | module (`.rs` file / `mod`) | intra-crate `use crate::` / `super::` / `self::` |
| Python (classes) | `scripts/pyclasses.mjs <dir>` | class (`ast`) | subclass → base (inheritance) |
| Terraform | `scripts/tfimports.mjs <dir>` | `resource` / `module` block, rendered as its **official AWS/Azure/GCP icon** | cross-resource reference (`aws_iam_role.x.arn`, `${...}`, `depends_on`) |
| Kubernetes | `scripts/k8simports.mjs <dir>` | manifest object (kind/name), rendered as its **official K8s kind icon** | Ingress→Service, Service→workload (selector), workload→ConfigMap/Secret/PVC, HPA→target |
| docker-compose | `scripts/composeimports.mjs <file-or-dir>` | service (name + image box) / named volume (cylinder) | `depends_on` / `links` / `volumes_from` / named-volume mounts |
| Terraform state (**live**) | `terraform show -json \| scripts/tfstate.mjs -` | **deployed** resource instance, rendered as its **official cloud icon** | recorded dependency (`depends_on` in state) |
| Docker (**live**) | `docker inspect $(docker ps -q) \| scripts/dockerimports.mjs -` | running container (name + image) / user network (ellipse) / named volume (cylinder) | container→network, container→volume, `links` / compose `depends_on` |
| SQL DDL | `scripts/sqlerd.mjs <file-or-dir>` | table (column list with PK/FK markers) | foreign key (crow's-foot, labeled with the FK column) |

```bash
node <this-skill-dir>/scripts/pyimports.mjs myproject -o graph.json
node <this-skill-dir>/scripts/autolayout.mjs graph.json -o diagram.drawio
```
...
**`--tune` (autolayout flag)**: lays the graph out in both directions (TB and LR), scores each (through-vertex routes ×20 + edge crossings ×10 + total edge length as tiebreak), and keeps the better one — report on stderr. `validate.mjs --score` prints the same style of readability score for a finished `.drawio`, for comparing variants.

**Density reduction is on by default** — this is the key to a readable result. Real import graphs are dense (asyncio: 33 modules / ~149 edges); without reduction they render as a hairball. Every importer applies **transitive reduction** (drops edges already implied by a longer path), which on asyncio cuts ~149 edges to ~46 and turns the hairball into a clean, traceable diagram. Pass `--no-reduce` to keep every edge.
...
The layout engine already routes edges orthogonally as part of its layout pass, so the result usually needs no further routing. When the output still has edges cutting across shapes (common in dense graphs with many inter-group edges), pass the produced `.drawio` through `drawio --layout libavoid` for a second routing-only pass — it keeps every autolayout vertex position and only reroutes the connectors around the shapes:

```bash
drawio --layout libavoid diagram.drawio -o diagram.drawio
```

This is a pure CLI operation, but requires draw.io CLI ≥ v30.

## Limitations

- **Placement is topological, not semantic** — the layout engine minimises edge crossings, which may put a node in a different column than you'd choose by hand. Re-export with the other `direction`, or hand-tune the produced XML afterwards (it's a normal `.drawio`).
