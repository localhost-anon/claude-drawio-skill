...
1. **Search the official shape index** (`scripts/shapesearch.mjs`) — 10,446 real
...
```bash
node <this-skill-dir>/scripts/shapesearch.mjs "aws lambda" --limit 5
node <this-skill-dir>/scripts/shapesearch.mjs "uml actor" --json
```
...
architecture" otherwise renders as generic boxes. `scripts/aiicons.mjs` resolves a
...
```bash
node <this-skill-dir>/scripts/aiicons.mjs "claude" --json        # CDN reference
node <this-skill-dir>/scripts/aiicons.mjs "openai" --embed        # self-contained
node <this-skill-dir>/scripts/aiicons.mjs --list                  # all brands
```
...
- **Data stores** common in RAG/LLM apps that lobe lacks (Qdrant, Redis,
  ...
  `scripts/shapesearch.mjs "<name> database"`.
