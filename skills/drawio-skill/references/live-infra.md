...
The IaC importers (`tfimports.mjs`, `k8simports.mjs`, `composeimports.mjs`) draw the
...
`autolayout.mjs` renders an editable `.drawio` with the same official icons.
...
```bash
terraform show -json | node <this-skill-dir>/scripts/tfstate.mjs - -o graph.json
node <this-skill-dir>/scripts/autolayout.mjs graph.json -o deployed.drawio
```
...
- Contrast with `tfimports.mjs` (the `.tf` files) when the user wants declared-vs-actual.

## 2. Running containers — Docker

```bash
docker inspect $(docker ps -q) | node <this-skill-dir>/scripts/dockerimports.mjs - -o graph.json
node <this-skill-dir>/scripts/autolayout.mjs graph.json -o running.drawio
```
...
No new script — `k8simports.mjs` already ingests `kubectl get ... -o json`. Ask for
...
kubectl get deploy,sts,ds,svc,ing,cm,secret,pvc,hpa -n <ns> -o json \
  | node <this-skill-dir>/scripts/k8simports.mjs - -o graph.json
node <this-skill-dir>/scripts/autolayout.mjs graph.json -o cluster.drawio
```
...
self-check, review, final export with `-e` + `repair_png.mjs`. A live snapshot is a
