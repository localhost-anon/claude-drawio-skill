...
[![GitHub stars](https://img.shields.io/github/stars/localhost-anon/claude-drawio-skill?style=flat&logo=github)](https://github.com/localhost-anon/claude-drawio-skill/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/localhost-anon/claude-drawio-skill?style=flat&logo=github)](https://github.com/localhost-anon/claude-drawio-skill/network/members)
[![Latest Release](https://img.shields.io/github/v/release/localhost-anon/claude-drawio-skill?logo=github)](https://github.com/localhost-anon/claude-drawio-skill/releases/latest)
[![Last Commit](https://img.shields.io/github/last-commit/localhost-anon/claude-drawio-skill?logo=github)](https://github.com/localhost-anon/claude-drawio-skill/commits/main)

[📖 Online Docs](https://localhost-anon.github.io/claude-drawio-skill/)
...
- **Visualize a codebase** — extract and auto-lay-out the structure of a Python / JS-TS / Go / Rust project (import graphs) or a Python class hierarchy — automatic placement, transitive reduction, nested module containers
...
- **Zero-config, zero dependencies** — runs from a single SKILL.md as a Claude Code skill; no MCP server, no background daemon, no external runtime dependency beyond Node itself
...
**Prerequisites:** Node >= 20 and the draw.io desktop CLI on `PATH`.
...
```bash
git clone https://github.com/localhost-anon/claude-drawio-skill && ln -s "$PWD/claude-drawio-skill/skills/drawio-skill" ~/.claude/skills/drawio-skill
```

**Updating:** `cd claude-drawio-skill && git pull` — the symlink always points at the current checkout. Release history in [CHANGELOG.md](CHANGELOG.md).
...
node scripts/tfimports.mjs   ./infra      -o graph.json   # Terraform → AWS/Azure/GCP icons
node scripts/k8simports.mjs  ./manifests  -o graph.json   # K8s YAML/JSON → kind icons
node scripts/composeimports.mjs compose.yml -o graph.json # services + named volumes

# Live infrastructure — draw what's ACTUALLY running / deployed
terraform show -json          | node scripts/tfstate.mjs -      -o graph.json  # deployed cloud
docker inspect $(docker ps -q)| node scripts/dockerimports.mjs -  -o graph.json  # running containers
kubectl get all,ing,cm,secret,pvc -o json | node scripts/k8simports.mjs - -o graph.json  # live cluster

# Data & interactions
node scripts/sqlerd.mjs      schema.sql   -o graph.json   # SQL DDL → ER diagram
node scripts/ciimports.mjs . -o graph.json              # GitHub Actions + GitLab CI -> pipeline DAG
node scripts/openapiimports.mjs openapi.yaml -o graph.json # OpenAPI/Swagger → API diagram (by method)
node scripts/seqlayout.mjs   seq.json  -o sequence.drawio # sequence diagram, direct to .drawio
node scripts/c4.mjs          c4.json   -o c4.drawio       # C4 model, multi-page + drill-down

# Diff two diagrams / snapshots → colour-coded "what changed"
node scripts/drawiodiff.mjs old.drawio new.drawio -o graph.json # +added -removed ~changed

# Architecture time-lapse → self-contained HTML player of how a codebase grew
node scripts/timelapse.mjs src --importer pyimports # → architecture-evolution.html

# Reverse: describe an existing .drawio as structured Markdown (README / PR summary)
node scripts/explain.mjs    architecture.drawio -o architecture.md

# Diagram → PowerPoint deck (one page per slide; C4 model → presentation)
node scripts/drawio2pptx.mjs c4.drawio -o c4.pptx

# Interactive HTML viewer — pan/zoom/search/tabs + working drill-down links, one file
node scripts/drawiohtml.mjs c4.drawio -o c4.html

# Animated data-flow SVG — edges "flow" (marching ants); renders on GitHub
node scripts/svgflow.mjs    architecture.drawio -o flow.svg

# Reverse: .drawio → Mermaid flowchart (diagrams-as-code GitHub renders)
node scripts/drawio2mermaid.mjs architecture.drawio --fenced -o arch.md

# Language variant: extract labels → translate values → apply (layout untouched)
node scripts/relabel.mjs architecture.drawio --extract -o labels.json
node scripts/relabel.mjs architecture.drawio --map labels.json -o architecture_cn.drawio

# Re-theme an existing .drawio with a style preset (e.g. dark mode)
node scripts/restyle.mjs architecture.drawio --preset dark

# Colour an existing .drawio by data → cost / latency / traffic heat map
node scripts/heatmap.mjs    architecture.drawio -m latency.csv --size -o hot.drawio
...
node scripts/raster2drawio.mjs whiteboard-graph.json -o out.drawio

# Watch a diagram build itself, node by node → HTML player (+ optional GIF)
node scripts/buildup.mjs architecture.drawio --gif build.gif  # → buildup.html

# Big diagram → boardroom exec summary (clustered) + click-to-drill-down to full
node scripts/compress.mjs  big.drawio -o exec.drawio

# Decision-tree flowchart → click-through HTML triage runbook (no draw.io CLI needed)
node scripts/runbook.mjs   triage.drawio -o triage.html

# CI: render base/head/diff PNGs + Markdown report for every .drawio a PR changed
node scripts/prdiff.mjs --base origin/main --head HEAD -o drawio-pr/report.md

# Tube-Map Mode — restyle a pipeline / journey as a metro / subway map
node scripts/tubemap.mjs metro.json -o metro.drawio
```
...
| **Diagram diff** | `drawiodiff.mjs` compares two `.drawio` (or two live snapshots) into one colour-coded graph — added=green, removed=red, changed=orange — so you can see architecture / infra **drift** at a glance |
| **Language variants** | `relabel.mjs` swaps every label via a JSON map with layout/styles/ids untouched — `--extract` dumps all labels, translate the values, `--map` applies them. One diagram → EN + CN twins for bilingual docs |
| **Re-theme** | `restyle.mjs` applies a style preset (built-in `dark`/`corporate`/… or your own) to an *existing* `.drawio` — palette remapped by hue so same-colored nodes stay grouped; layout and edge routing untouched |
| **Metric heat map** | `heatmap.mjs` recolours an existing `.drawio` from a CSV/JSON of per-node values — cost / latency / traffic / error-rate shaded low→high on a gradient (optional size-by-value + legend), matched by cell id or label |
| **Architecture time-lapse** | `timelapse.mjs` re-runs an importer across a repo's git history and assembles a self-contained HTML player — watch modules & edges appear over time (▶ play / ‹ › step) |
| **Diagram → Markdown** | `explain.mjs` reverses a `.drawio` into a structured description — components by tier, relations, per-page for C4 — for dropping an architecture summary into a README or PR |
| **Interactive viewer** | `drawiohtml.mjs` publishes a `.drawio` as one self-contained HTML — page tabs, drag-pan, wheel-zoom, node search, and a C4 model's drill-down links keep working. Share the file; no draw.io, no server |
| **Diagram → PowerPoint** | `drawio2pptx.mjs` turns a multi-page diagram into a 16:9 deck (one page per slide, page name as title) — a C4 model becomes a ready-to-present slideshow |
| **Animated data-flow** | `svgflow.mjs` makes a diagram's edges *flow* (marching-ants animation along each arrow) — a self-contained looping SVG that renders on GitHub, in docs, or as a slide background |
| **Diagram → Mermaid** | `drawio2mermaid.mjs` converts a `.drawio` into a Mermaid `flowchart` (containers → subgraphs, edge labels kept) — paste it into Markdown as diagrams-as-code that GitHub renders natively |
| **Sequence engine** | `seqlayout.mjs` computes lifeline / activation-bar / arrow geometry from a message list — no hand placement |
| **Auto-layout** | The built-in ELK layout engine places nodes and routes orthogonal edges *around* them — removes the manual-coordinate ceiling for large graphs. `--tune` tries both directions and keeps the more readable one |
...
| **Deterministic validator** | `validate.mjs` lints the `.drawio` (dangling edges, duplicate ids, overlaps) before the visual self-check |

Auto-layout is built in — no external dependency to install. Full format + flag reference in [references/autolayout.md](skills/drawio-skill/references/autolayout.md). Regenerate, validate (`--strict` gate) and render headlessly in CI: [docs/CI.md](docs/CI.md).
...
<p align="center">
  <img src="assets/shape-search-example.png" width="900" alt="Serverless AWS architecture built from official draw.io icons resolved by shapesearch.mjs">
</p>

<sub>↑ A serverless AWS architecture — every icon is the real official draw.io shape resolved by <code>shapesearch.mjs</code>, not a hand-guessed <code>shape=</code> string.</sub>
...
draw.io ships **no** modern AI/LLM logos, so an LLM-app diagram renders as generic boxes. `aiicons.mjs` resolves a brand name to a draw.io image style for any of **321 logos** (OpenAI, Claude, Gemini, Mistral, Llama, Cohere, DeepSeek, Qwen, Ollama, LangChain, HuggingFace…) from [lobe-icons](https://github.com/lobehub/lobe-icons) (MIT), plus **18 data-store brands** (Redis, Postgres, MongoDB, Qdrant, Milvus, Supabase…) via [simple-icons](https://simpleicons.org) (CC0) for RAG stacks.
...
<p align="center">
  <img src="assets/ai-logos-example.png" width="900" alt="Multi-provider LLM app diagram with real AI brand logos resolved by aiicons.mjs">
</p>

<sub>↑ A multi-provider LLM app — every brand logo resolved by <code>aiicons.mjs</code>. Icons are referenced from the unpkg CDN by default (network needed at render time); <code>--embed</code> inlines them for offline use. Logos are trademarks of their owners, used for identification only.</sub>
...
| Sequence diagrams | ❌ hand-placed coordinates | ✅ deterministic geometry engine (`seqlayout.mjs`) |
| C4 model | ❌ | ✅ multi-page Context→Container→Component with click-to-drill-down |
| Auto-layout for large graphs | ❌ hand-places, overlaps | ✅ built-in ELK placement, ortho routing, nested containers |
...
| AI/LLM brand logos | ❌ none | ✅ 321 AI + 18 data-store logos via aiicons.mjs |
...
|---|---|---|---|---|
| **Approach** | Pure SKILL.md (Claude Code) | MCP servers / Claude Code plugin / Project | YAML DSL + CLI (MCP optional) | Claude Code plugin |
...
**Reach for something else instead when you need:**
- A casual, hand-drawn / whiteboard look, or freeform infinite-canvas sketching
- Diagrams-as-code that live in git and render directly in Markdown (e.g. Mermaid, PlantUML)

## ❤️ Credits

This project is a Node.js, Claude Code-only rewrite of
[Agents365-ai/drawio-skill](https://github.com/Agents365-ai/drawio-skill) (MIT licensed).
All credit for the original design, diagram presets, and shape-search approach goes to
that upstream project — this repo ports it to pure Node.js and narrows scope to Claude
Code only.
